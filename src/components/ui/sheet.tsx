...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-[hsl(var(--background)/0.85)] backdrop-blur-3xl border-white/10 p-6 shadow-2xl transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out flex flex-col",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ...
      {...props}
    >
      <SheetPrimitive.Close className="absolute right-5 top-5 rounded-full w-8 h-8 flex items-center justify-center bg-white/5 border border-white/10 opacity-70 hover:opacity-100 hover:bg-white/10 transition-all">
        <X className="h-4 w-4" />
...
export {
  ...
  SheetClose, SheetContent, SheetHeader, SheetFooter,
